...
export const Settings = ({ className, buttonClassName }: { className: string, buttonClassName: string }) => {
  ...
  const sectionWrapper: ClassName = "hover:bg-accent/50 flex items-start gap-3 rounded-lg border p-3 has-[[aria-checked=true]]:border-blue-600 has-[[aria-checked=true]]:bg-blue-50 dark:has-[[aria-checked=true]]:border-blue-900 dark:has-[[aria-checked=true]]:bg-blue-950"
  const sectionCheckBox: ClassName = "data-[state=checked]:border-blue-600 data-[state=checked]:bg-blue-600 data-[state=checked]:text-white dark:data-[state=checked]:border-blue-700 dark:data-[state=checked]:bg-blue-700"
  const compressionPercent = Math.round(config.compression * 100)

  const updateOperationToggle = (operation: keyof Configuration["operations"]) => (checked: boolean) =>
    setConfig((old) => ({
      ...old,
      operations: {
        ...old.operations,
          ...
          <div className="grid gap-2">
            <div className={sectionWrapper}>
              ...
              <div className="grid gap-1.5 font-normal w-full">
                <p onClick={() => updateOperationToggle("compress")(!config.operations.compress)} onKeyUp={() => { }} className="text-sm leading-none font-medium cursor-pointer">
                  Compression
                </p>
                  ...
                  && <div className="flex flex-row content-between gap-4 flex-grow pt-4">
                    ...
                    <div className="flex flex-row gap-3 flex-grow">
                      <Slider
                        ...
                        max={99}
                        step={1}
                        onValueChange={(val) => setConfig((old) => ({ ...old, compression: (val[0] / 100) }))}
                      />
            ...
            <div className={sectionWrapper}>
              ...
              <div className="grid gap-1.5 font-normal">
                <p onClick={() => updateOperationToggle("resize")(!config.operations.resize)} onKeyUp={() => { }} className="text-sm leading-none font-medium cursor-pointer">
                  Resize
                </p>
                {config.operations.resize && <div className="flex flex-col gap-3 mt-3">
